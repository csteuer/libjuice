...
static void picohash_init_hmac(picohash_ctx_t *ctx, void (*initf)(picohash_ctx_t *),
...
inline void _picohash_sha1_final(_picohash_sha1_ctx_t *s, void *digest) {
	...
	{ // Swap byte order back
		...
		for (i = 0; i < 5; i++) {
			s->state[i] =
			    (((s->state[i]) << 24) & 0xff000000) | (((s->state[i]) << 8) & 0x00ff0000) |
...
#define _picohash_sha256_s(x, y)                                                                   \
	(((((uint32_t)(x)&0xFFFFFFFFUL) >> (uint32_t)((y)&31)) |                                       \
	  ((uint32_t)(x) << (uint32_t)(32 - ((y)&31)))) &                                              \
	 0xFFFFFFFFUL)
#define _picohash_sha256_r(x, n) (((x)&0xFFFFFFFFUL) >> (n))
#define _picohash_sha256_sigma0(x)                                                                 \
	(_picohash_sha256_s(x, 2) ^ _picohash_sha256_s(x, 13) ^ _picohash_sha256_s(x, 22))
#define _picohash_sha256_sigma1(x)                                                                 \
	(_picohash_sha256_s(x, 6) ^ _picohash_sha256_s(x, 11) ^ _picohash_sha256_s(x, 25))
#define _picohash_sha256_gamma0(x)                                                                 \
	(_picohash_sha256_s(x, 7) ^ _picohash_sha256_s(x, 18) ^ _picohash_sha256_r(x, 3))
#define _picohash_sha256_gamma1(x)                                                                 \
	(_picohash_sha256_s(x, 17) ^ _picohash_sha256_s(x, 19) ^ _picohash_sha256_r(x, 10))
...
static inline void _picohash_sha256_compress(_picohash_sha256_ctx_t *ctx, unsigned char *buf) {
	...
	/* fill W[16..63] */
	for (i = 16; i < 64; i++)
		W[i] = _picohash_sha256_gamma1(W[i - 2]) + W[i - 7] + _picohash_sha256_gamma0(W[i - 15]) +
		       W[i - 16];
...
static inline void _picohash_sha256_do_final(_picohash_sha256_ctx_t *ctx, void *digest,
                                             size_t len) {
...
inline void picohash_final(picohash_ctx_t *ctx, void *digest) { ctx->_final(ctx, digest); }

inline void picohash_reset(picohash_ctx_t *ctx) { ctx->_reset(ctx); }
...
inline void picohash_init_hmac(picohash_ctx_t *ctx, void (*initf)(picohash_ctx_t *),
                               const void *key, size_t key_len) {
	initf(ctx);
